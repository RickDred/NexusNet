...
func (u UserModel) Insert(user *User) error {
	query := `
	INSERT INTO users (name, email, role, password_hash, activated, description)
	VALUES ($1, $2, $3, $4, $5, $6)
	RETURNING id, created_at`
	args := []any{user.Name, user.Email, "user", user.Password.hash, user.Activated, user.Description}
...
func (u UserModel) GetByEmail(email string) (*User, error) {
	query := `
	SELECT id, created_at, name, email, role, password_hash, activated, description
	FROM users
	WHERE email = $1`
	var user User
	ctx, cancel := context.WithTimeout(context.Background(), 3*time.Second)
...
func (u UserModel) Update(user *User) error {
	query := `
	UPDATE users
	SET name = $1, email = $2, password_hash = $3, activated = $4, description = $5, role = $6
	WHERE id = $7
...
func (u UserModel) GetForToken(tokenScope, tokenPlaintext string) (*User, error) {
	...
	query := `
	SELECT users.id, users.created_at, users.name, users.email, users.role, users.password_hash, users.activated, users.description
	FROM users
	INNER JOIN tokens
...
func (u UserModel) GetAll(name string, filters Filters) ([]*User, Metadata, error) {
